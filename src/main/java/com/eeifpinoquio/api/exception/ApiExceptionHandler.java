...
class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	private Problema criarProblemaBuilder(HttpStatus status, TipoProblema tipoProblema, String detalhe) {
		return Problema.builder().status(status.value()).type(tipoProblema.getUri()).title(tipoProblema.getTitulo())
				.detail(detalhe).timestamp(OffsetDateTime.now()).build();
	}
	...
	private String paramentrosInvalidos(List<FieldError> campos) {
		return campos.stream().map(camp -> camp.getField() + " " + camp.getDefaultMessage()).limit(1)
